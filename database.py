...
class frame_change(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.frames = {}
        for F in (main_interface, employee_interface, pat_interface, pat_choices, new_pat_interface, info_update, acc_update, bill_update, appt_update, rec_update):
...
class pat_interface(tk.Frame):
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        self.controller = controller
        label = tk.Label(self, text = 'Enter Patient ID:', font = controller.title_font)
        label.grid(columnspan = 2)


        entry = tk.Entry(self)
        entry.grid(row = 1, column = 0)
        cursor.execute('SELECT * FROM PATIENT WHERE PATIENT_ID =\''+entry.get()+'\'')
        myresult=cursor.fetchall()
        for row in myresult:
            print(row)
        button = tk.Button(self, text = 'Enter', command = lambda: setPid(self,parent,controller,entry.get()))
        button.grid(row = 1 , column = 1)

        # Use entry.get() to acquire input, does not have to be declared to a variable
        add_button = tk.Button(self, text = 'List of Patients', width = 16, command = lambda: patients())
        add_button.grid(columnspan = 2)
        add_button = tk.Button(self, text = 'Add new Patient', width = 16, command = lambda: controller.show_frame("new_pat_interface"))
        add_button.grid(columnspan = 2)
        return_button = tk.Button(self, text = 'Return to Main Menu', width = 16, command = lambda: controller.show_frame("main_interface"))
...
class new_pat_interface(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, text = 'New Patient Information', font = controller.title_font)
        label.grid(columnspan = 2, rowspan = 2)


        info_button = tk.Button(self, text = 'Patient Info.', width = 10,command = lambda: controller.show_frame("info_update"))
        info_button.grid(row = 2, column = 0)
        acc_button = tk.Button(self, text = 'Account Info.', width = 10,command = lambda: controller.show_frame("acc_update"))
        acc_button.grid(row = 2, column = 1)
        appt_button = tk.Button(self, text = 'Appointments', width = 10, command = lambda: controller.show_frame("appt_update"))
        appt_button.grid(row = 3, column = 0)
        bill_button = tk.Button(self, text = 'Bill', width = 10, command = lambda: controller.show_frame("bill_update"))
        bill_button.grid(row = 3, column = 1)
        rec_button = tk.Button(self, text = 'Records', width = 10, command = lambda: controller.show_frame("rec_update"))
        rec_button.grid(row = 4, column = 0)
...
class info_update(tk.Frame):
    def __init__(self, parent, controller):
        ...
        def save():
            ...
            cursor.execute('INSERT INTO PATIENT(PATIENT_ID, FNAME, LNAME, DOB, ST_ADDR, ZIP, STATE) VALUES('+patientID_entry.get()+', \''+fname_entry.get()+'\', \''+lname_entry.get()+'\', \''+dob_entry.get()+'\', \''+street_entry.get()+'\', '+zip_entry.get()+', \''+state_entry.get()+'\');')
        ...
        tk.Button(self, text='Save', command=lambda: save()).grid(row=9, column=1, sticky=tk.W, pady=4)


        return_button = tk.Button(self, text = 'Return to select Patient', command = lambda: controller.show_frame("new_pat_interface"))
        return_button.grid(columnspan = 2)
...
class acc_update(tk.Frame):
    def __init__(self, parent, controller):
        ...
        def save():
            ...
            cursor.execute('INSERT INTO PATIENT_ACCOUNT(PATIENT_ID, ACCOUNT_NUM, INSURANCE, ACCT_PASSWORD) VALUES('+patientID_entry.get()+', \''+accNum_entry.get()+'\', \''+insurance_entry.get()+'\', \''+pw_entry.get()+'\');')
            # cursor.execute('''
...
class bill_update(tk.Frame):
    def __init__(self, parent, controller):
        ...
        def save():
            ...
            cursor.execute('INSERT INTO BILL(ACCOUNT_NUM, COPAY, AMOUNT, DUE_DATE) VALUES('+acct_num_entry.get()+', \''+copay_entry.get()+'\', \''+amount_entry.get()+'\', \''+due_entry.get()+'\');')
...
class appt_update(tk.Frame):
    def __init__(self, parent, controller):
        ...
        def save():
            ...
            cursor.execute('INSERT INTO APPOINTMENTS(TEST, ACCOUNT_NUM, DOCTOR_ID, RECEPT_ID) VALUES('+test_entry.get()+', \''+accNum_entry.get()+'\', \''+doctor_entry.get()+'\', \''+recept_entry.get()+'\');')
...
class rec_update(tk.Frame):
    def __init__(self, parent, controller):
        ...
        def save():
            ...
            cursor.execute('INSERT INTO PATIENT_ACCOUNT(ACCOUNT_NUM, DOCUMENT) VALUES('+acct_num_entry.get()+', \''+doc_entry.get()+'\');')
...
def patients():
    ...
    finalmsg=''
    for row in myresult:  
        msg = "First Name: "+ row[0]+ "\tLast Name: "+ row[1]+ "\tPatient ID: "+ row[2]+"\t"
        finalmsg=finalmsg+"\t\n"+msg
...
def info():
    ...
    cursor.execute('SELECT FNAME, LNAME FROM PATIENT WHERE PATIENT_ID =\''+pid+'\'')
    arr=cursor.fetchall()
    for row in arr:
        name=row[0]+' '+row[1]
...
def appt():
    cursor.execute('SELECT * FROM APPOINTMENTS WHERE ACCOUNT_NUM=(SELECT ACCOUNT_NUM FROM PATIENT_ACCOUNT WHERE PATIENT_ID =\''+pid+'\')')
    myresult=cursor.fetchall()
    msg=''
    for row in myresult:   
        msg = "Test: "+ row[0]+ "\nAccount Number: "+ row[1]+ "\nDoctor ID: "+ row[2]+"\nReceipt ID: "+ row[3]
...
def bill():
    cursor.execute('SELECT * FROM BILL WHERE ACCOUNT_NUM=(SELECT ACCOUNT_NUM FROM PATIENT_ACCOUNT WHERE PATIENT_ID=\''+pid+'\')')  
    myresult=cursor.fetchall()
...
def rec():    
    cursor.execute('SELECT * FROM RECORD WHERE ACCOUNT_NUM=(SELECT ACCOUNT_NUM FROM PATIENT_ACCOUNT WHERE PATIENT_ID =\''+pid+'\')')
    myresult=cursor.fetchall()
    msg=''
    for row in myresult:   
            msg = "Account Number: "+ row[0]+ "\nRecords: "+ row[1]
